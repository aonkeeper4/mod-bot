...
class Suggestion:  # yes
    def __init__(self,
        ...
        self.title = title
        self.body = body
        self.id = s_id if s_id is not None else 's' + secrets.token_hex(
            4)  # suggestion id
    ...
    def to_embed(self, *, insert=False, colour=discord.Colour.dark_teal()):
        ...
        embed = discord.Embed(title=('Inspect - ' if insert else '') +
                              'Suggestion #' + str(self.position),
                              colour=colour)
        embed.add_field(name=self.title, value=self.body)
        embed.set_footer(
            text=
            f'{self.id} | Created at {self.created_at} | Tags: {tag_string}')
        return embed
    ...
    def from_string(string):
        ...
        args[2] = datetime.strptime(
            args[2],
            '%d/%m/%Y %H:%M:%S') if args[2] > '' else datetime.now().strftime(
                "%d/%m/%Y %H:%M:%S"
            )  # gets created_at from string. this was painful
        args[5] = args[5].split(':')
        return Suggestion(*args)  # woo list unpacking

    def __contains__(self, other):  # for search feature
        return (other in self.title) or (other == self.id) or (other
                                                               in self.body)
...
def get_filter_vars(
        s_filter):  # TODO: get rid of all of the Suggestion.from_string
    ...
    if s_filter is None:
        ...
        for q in queries:
            ...
            if args_list[0] == "tags":  # use match statement?
                tags = args_list[1:]
                f = lambda kv: any(tag in Suggestion.from_string(kv[1]).tags
                                   for tag in tags)
                ...
                if date[-1] in "ymwdh":
                    ...
                    total_date_time = base_dates_dict[timescale] * int(
                        date[:-1])
                else:
                    total_date_time = datetime.strptime(date, "%d/%m/%Y")
                f = lambda kv: datetime.now(
                ) - total_date_time <= datetime.strptime(
                    Suggestion.from_string(kv[1]).created_at,
                    "%d/%m/%Y %H:%M:%S")
                ...
                search = args_list[1]
                f = lambda kv: any(word in Suggestion.from_string(kv[1])
...
def safe_suggestion_lookup(coro):  # finally, a good use for decorators
    ...
    async def _wrapper(*args, **kwargs):
        try:
            ...
            await send_embed(ctx, 'Error', 'Error - invalid suggestion id',
                             discord.Colour.red())
            return
...
class SuggestionsCog(commands.Cog
                     ):  # cog to hold all suggestions-related commands
    def __init__(self, bot):
        self.bot = bot

    @commands.command(
        help=
        f'Creates a new suggestion and sends it to the #{ADMIN_CHANNEL} channel.'
    )
    async def suggest(self, ctx):
        await send_embed(ctx, 'Suggestion title',
                         'Please type your suggestion title',
                         discord.Colour.dark_teal())
        title = await wait_for_message(
            ctx, lambda msg: msg.author == ctx.message.author and len(
                msg.content) <= 30)
        await send_embed(ctx, 'Suggestion body',
                         'Please type the rest of your suggestion',
                         discord.Colour.dark_teal())
        body = await wait_for_message(
            ctx, lambda msg: msg.author == ctx.message.author)
        suggestion = Suggestion(title.content, body.content)
        embed = suggestion.to_embed()
        admin_channel = discord.utils.get(ctx.message.guild.channels,
                                          name=ADMIN_CHANNEL)
        await admin_channel.send(embed=embed)
        await send_embed(
            ctx, 'Suggestion sent',
            'Suggestion created successfully! It has been sent to the admins for them to review.',
            discord.Colour.dark_teal())

    @commands.command(aliases=['list'],
                      help='View all suggestions - can be used with a filter')
    async def _list(self, ctx, *, s_filter=None):
        ...
        async with ctx.typing():
            ...
            all_suggestions = list(filter(check,
                                          db.items()))  # TODO: use generators
            if len(all_suggestions) == 0:
                await send_embed(ctx, f'Suggestions - {filter_str}',
                                 '```Nothing was found by that query.```',
                                 discord.Colour.gold())
                return
            suggestion_pages = []
            for i in range(0, len(all_suggestions), 6):
                try:
                    suggestion_pages.append(all_suggestions[i:i + 6])
                except IndexError:
                    suggestion_pages.append(
                        all_suggestions[i:i + len(all_suggestions) %
                                        6])  # suggesstion numbers for menu
            suggestion_strs = []
            for suggestion_page in suggestion_pages:
                suggestion_str = '{0:^18}|{1:^31}\n'.format('ID', 'Title')
                suggestion_str += '-' * 18 + '+' + '-' * 31 + '\n'
                suggestion_str += '\n'.join([
                    '{0:^18}|{1:^31}'.format(s_id,
                                             Suggestion.from_string(s).title)
                    for s_id, s in suggestion_page
                ])  # display suggestion previews
                suggestion_strs.append(suggestion_str)
            current_page = 0

        async def setup_msg(ctx, current_page, msg=None):
            emoji_list = ['◀'] * (current_page > 0) + [
                '1️⃣', '2️⃣', '3️⃣', '4️⃣', '5️⃣', '6️⃣'
            ][:len(suggestion_pages[current_page])] + ['▶'] * (
                current_page < len(suggestion_pages) - 1
            )  # gets emojis for menu
            msg = await send_embed(
                ctx,
                f'Suggestions - {filter_str} ({len(all_suggestions)} results)',
                f'```{suggestion_strs[current_page]}```',
                discord.Colour.gold(),
                True,
                f'Page {current_page+1}/{len(suggestion_pages)}' +
                '\u3000' * 30,
                edit=msg)  # actually display the page
        ...
        async def wait_for_reactions(
                ctx, current_page, msg,
                emoji_list):  # recursive function so the menu is useful
            try:
            ...
            if reaction.emoji == '◀':  # dec page
                ...
                # add back button?
                n_emoji_list = emoji_list[(
                    current_page > 0
                ):] if current_page < len(suggestion_pages) else emoji_list[(
                    current_page > 0):-1]
                number = n_emoji_list.index(
                    reaction.emoji)  # no more ord magic
                embed = Suggestion.from_string(
                    suggestion_pages[current_page][number][1]).to_embed(
                        insert=True, colour=discord.Colour.gold())
                await msg.clear_reactions()
    ...
    async def clear(self, ctx):
        for key in db:
            del db[key]
        await send_embed(ctx, 'Cleared', 'Cleared all suggestions! :boom:',
    ...
    async def delete(self, ctx, s_id):
        del db[s_id]
        await send_embed(ctx, 'Suggestion deleted',
                         'Deleted suggestion ' + s_id, discord.Colour.green())

    @commands.command(help='Adds a tag to a suggestion by id')
    @safe_suggestion_lookup
    async def tag(self, ctx, s_id, tag):
        tagged_suggestion = Suggestion.from_string(db[s_id]).add_tag(
            tag)  # garbage collection
        tagged_suggestion.add()
        await send_embed(ctx, 'Tag added',
                         f'Tag {tag} added to suggestion {s_id}',
                         discord.Colour.green())

    @commands.command(help='Inspects a suggestion by id')
    @safe_suggestion_lookup
    async def inspect(self, ctx, s_id):
        embed = Suggestion.from_string(db[s_id]).to_embed(
            insert=True, colour=discord.Colour.teal())
        await ctx.send(embed=embed)

    @commands.command(
        help=
        'Populates suggestions database with random suggestions. This does not send the suggestions to the admins as it is a debug command.'
    )
    async def populate(self, ctx, number=25):
        if number > 100:
            await send_embed(ctx, 'Error',
                             'Error - too many suggestions to populate',
                             discord.Colour.red())
            return
        async with ctx.typing():
            for i in range(number):
                ...
                suggestion.add()
        await send_embed(
            ctx, 'Suggestions populated',
            f'The suggestions database was populated with {number} suggestions',
            discord.Colour.blue())
